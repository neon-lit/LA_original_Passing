...
function Caption({
  ...
  const passingLive = theme.id === "passing-live";
  ...
  return (
    <div style={{ textAlign: align, position: "relative", width: "100%" }}>
      <EditableText
        ...
        placeholder="LABEL"
        style={{
          fontSize: unit * (passingLive ? 0.024 : 0.028),
          fontWeight: 600,
          letterSpacing: unit * (passingLive ? 0.003 : 0.0015),
          color: accent,
...
function PassingHeadline({
  ...
  const lines = (value || "Headline goes here").split("\n");
  return (
    <div
      contentEditable={editable}
      suppressContentEditableWarning
      ...
      style={{
        ...
        cursor: editable ? "text" : "default",
        color,
        fontFamily: '"Iowan Old Style", "Hiragino Mincho ProN", "Yu Mincho", Georgia, serif',
        fontSize: fontSize ?? unit * 0.105,
        fontWeight: 400,
        lineHeight: 1.04,
        letterSpacing: -unit * 0.00035,
    ...
    >
      {lines.map((line, index) => (
        <React.Fragment key={`${line}-${index}`}>
          {index > 0 && <br />}
          <span style={{ fontStyle: index === lines.length - 1 ? "italic" : "normal" }}>{line}</span>
        </React.Fragment>
      ))}
    </div>
  );
...
function SlideBackground({
  ...
  if (theme.id === "passing-live") {
    const base = inverted ? "#1F1C1A" : "#0A0A0B";
    return (
      <div
        style={{
          position: "absolute",
          ...
          overflow: "hidden",
          background: [
            "radial-gradient(circle at 72% 16%, rgba(232,185,122,0.30) 0%, rgba(138,90,51,0.17) 10%, transparent 29%)",
            "radial-gradient(ellipse at 12% 50%, rgba(26,32,38,0.94) 0%, transparent 55%)",
            `linear-gradient(145deg, #1F1C1A 0%, #10141B 46%, ${base} 100%)`,
          ].join(", "),
          color: theme.fg,
        }}
      >
        <div
          style={{
            ...
            left: cW * 0.63,
            top: cH * 0.095,
            background: "rgba(232,185,122,0.16)",
            filter: `blur(${cW * 0.045}px)`,
          }}
        />
        <div
          style={{
